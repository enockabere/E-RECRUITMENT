    ...
    def post(self, request):
        try:
            ...
            if ("authenticated" not in request.session and name == "") or (
                ...
                return JsonResponse(
                    {"success": False, "error": "email and name cannot be empty"}
                )
            send_mail = self.send_message(
                name, reply_email, subject, message, email_template, recipient_email
            )
            if send_mail == True:
                return JsonResponse(
                    {
...
class Profile(UserObjectMixins, View):
    async def get(self, request):
        try:
            ...
            authenticated = await sync_to_async(request.session.__getitem__)(
                "authenticated"
            )
            Profile = True
            Dashboard = False
            ...
            async with aiohttp.ClientSession() as session:
                personal_details = asyncio.ensure_future(
                    self.simple_one_filtered_data(
                        session, "/QyApplicants", "No_", "eq", applicantNo
                    )
                )
                get_country = asyncio.ensure_future(
                    self.simple_fetch_data(session, "/CountryRegion")
                )

                get_tribes = asyncio.ensure_future(
                    self.simple_fetch_data(session, "/QyKenyanTribes")
                )
                field_of_study = asyncio.ensure_future(
                    self.simple_fetch_data(session, "/QyFieldsOfStudy")
                )

                qualifications = asyncio.ensure_future(
                    self.simple_fetch_data(session, "/QyQualificationCodes")
                )

                job_industries = asyncio.ensure_future(
                    self.simple_fetch_data(session, "/QyJobIndustries")
                )

                pro_bodies = asyncio.ensure_future(
                    self.simple_fetch_data(session, "/QyProfessionalBodies")
                )
                get_counties = asyncio.ensure_future(
                    self.simple_fetch_data(session, "/QyCounties")
                )
                get_func_areas = asyncio.ensure_future(
                    self.simple_fetch_data(session, "/QyFunctionalAreas")
                )
                pro_courses = asyncio.ensure_future(
                    self.simple_fetch_data(session, "/QyProfessionalCourses")
                )
...
class AcademicQualifications(UserObjectMixins, View):
    def get(self, request):
        try:
            ...
            qualifications = self.one_filter(
...
class QyApplicantJobExperience(UserObjectMixins, View):
    def get(self, request):
        try:
            Applicant_No_ = request.session["No_"]
            experience = self.one_filter(
                "/QyApplicantJobExperience", "Applicant_No_", "eq", Applicant_No_
            )
...
class QyApplicantProfessionalMemberships(UserObjectMixins, View):
    def get(self, request):
        try:
            Applicant_No_ = request.session["No_"]
            pro_memberships = self.one_filter(
...
class QyApplicantHobbies(UserObjectMixins, View):
    def get(self, request):
        try:
            Applicant_No_ = request.session["No_"]
            hobbies = self.one_filter("/QyApplicantHobbies", "No_", "eq", Applicant_No_)
...
class QyApplicantReferees(UserObjectMixins, View):
    def get(self, request):
        try:
            Applicant_No_ = request.session["No_"]
            referees = self.one_filter(
                "/QyApplicantReferees", "No", "eq", Applicant_No_
            )
...
class FnApplicantAcademicQualification(UserObjectMixins, View):
    def post(self, request):
        try:
            ...
            response = self.make_soap_request(
                "FnApplicantAcademicQualification",
                applicantNo,
                lineNo,
                startDate,
                endDate,
...
class JobExperience(UserObjectMixins, View):
    def post(self, request):
        try:
            ...
            other_hierarchyLevel = request.POST.get("hierarchyLevel")
            ...
            if not other_hierarchyLevel:
                other_hierarchyLevel = ""
...
class FnApplicantProfessionalMembership(UserObjectMixins, View):
    def post(self, request):
        try:
            ...
            response = self.make_soap_request(
                "FnApplicantProfessionalMembership",
                applicantNo,
                lineNo,
                professionalBody,
                membershipNo,
...
class FnApplicantHobby(UserObjectMixins, View):
    def post(self, request):
        try:
            ...
            response = self.make_soap_request(
                "FnApplicantHobby", applicantNo, lineNo, hobby, myAction
            )
