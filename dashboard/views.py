    ...
    def get(self, request, pk, no):
        try:
            ...
            ProfessionalCourses = self.one_filter(
                "/QyJobProfessionalCourses", "Job_ID", "eq", pk
            )
    ...
    def get(self, request):
        try:
            ...
            open_vacancy = [
                ...
                if job["Submitted_To_Portal"] == True
                and datetime.strptime(job["End_Date"], "%Y-%m-%d") >= current_datetime
                and job["Job_ID"] not in submitted_list
            ]
            ...
            async with aiohttp.ClientSession() as session:
                task_get_docs = asyncio.ensure_future(
                    self.simple_one_filtered_data(
                        session, "/QyJobAttachments", "Job_ID", "eq", pk
                    )
                )
                task_get_attached = asyncio.ensure_future(
                    self.simple_one_filtered_data(
                        session, "/QyDocumentAttachments", "No_", "eq", no
                    )
                )
                ...
                for d in required_files:
                    d["Attachment"] = d["Attachment"].replace(" ", "_")
                required_files = [
                    d
                    for d in required_files
                    if not any(a["File_Name"] == d["Attachment"] for a in attached)
                ]
...
class Attachments(UserObjectMixins, View):
    async def get(self, request, no):
        try:
            Attachments = []
            async with aiohttp.ClientSession() as session:
                task_get_leave_attachments = asyncio.ensure_future(
                    self.simple_one_filtered_data(
                        session, "/QyDocumentAttachments", "No_", "eq", no
                    )
                )
...
class FnWithdrawJobApplication(UserObjectMixins, View):
    def post(self, request):
        try:
            ...
            response = self.make_soap_request(
                "FnWithdrawJobApplication", applicantNo, needCode
            )
